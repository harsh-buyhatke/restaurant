...
fetch('/api/orderHistory', {
    ...
    .then(orderhistory => {
        ...
        for (let i = 0; i < sz; ++i) {
            ...
            newDiv.append(`${orderhistory.data[0][i].address} : ${orderhistory.data[0][i].items} :  ${orderhistory.data[0][i].total} `)
            const input = document.createElement("input")
            input.setAttribute("type", "number");
            input.setAttribute("name", `${orderhistory.data[0][i].items}`);
